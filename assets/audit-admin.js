...
async function loadOptions(){const r=await fetch('/.netlify/functions/audit-filter-options');const d=await r.json();if(!r.ok||!d.ok)return;const ds=document.getElementById('dataset');const cs=document.getElementById('consultant');ds.innerHTML="<option value=''>All specimen types</option>"+(d.specimen_types||[]).map(o=>`<option value='${o.dataset_id}'>${o.label}</option>`).join('');cs.innerHTML="<option value=''>All consultants</option>"+(d.consultants||[]).map(c=>`<option value='${c}'>${c}</option>`).join('');const cf=document.getElementById('consultantFrom');if(cf) cf.innerHTML="<option value=''>Select existing name</option>"+(d.consultants||[]).map(c=>`<option value='${c}'>${c}</option>`).join('');}


async function normalizeConsultant(){const from_name=document.getElementById('consultantFrom').value;const to_name=document.getElementById('consultantTo').value.trim();const edited_by=document.getElementById('consultantEditedBy').value.trim();if(!from_name||!to_name||!edited_by){alert('from/to/edited by required');return;}const res=await fetch('/.netlify/functions/audit-consultant-normalize',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({from_name,to_name,edited_by})});const data=await res.json();if(!res.ok||!data.ok){alert(data.error||'normalize failed');return;}alert(`Updated ${data.updated_count} rows`);await loadOptions();await search();}
...
const nb=document.getElementById('normalizeConsultant'); if(nb) nb.addEventListener('click',normalizeConsultant);
