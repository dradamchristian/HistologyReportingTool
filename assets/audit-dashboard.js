...
['M0/M1',`${sb.m0_cases||0}/${sb.m1_cases||0}`,'M stage distribution']
 ...
 el('cases').querySelector('tbody').innerHTML=(d.cases||[]).map(r=>`<tr class='${(r.nodes_examined!==null&&Number(r.nodes_examined)<12)||r.crm_involved?'flag':''}'><td>${dateOnly(r.case_date)}</td><td>${r.consultant_name||''}</td><td>${r.tumour_site||''}</td><td>${[r.pt_stage,r.pn_stage,r.pm_stage].filter(Boolean).join(' ')}</td><td>${r.nodes_positive??''}/${r.nodes_examined??''}</td><td>${r.crm_involved?'R1':'R0'}${r.crm_distance_mm?` - CRM ${r.crm_distance_mm}mm`:''}${r.margin_longitudinal_involved?' / Prox involved':''}${r.margin_distal_involved?' / Distal involved':''}</td><td>Lymphatic: ${invStatus(r.lvi_present)} (${r.lymphatic_invasion_level||'n/a'}) / Perineural: ${invStatus(r.pni_present)} (${r.perineural_invasion_level||'n/a'}) / Venous: ${invStatus(r.emvi_present)} (${r.venous_invasion_level||'n/a'})</td></tr>`).join('');
