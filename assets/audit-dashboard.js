...
function benchCard(label,num,den,targetPct,mode='min'){const rate=den?Math.round((num*100)/den):0;const pass=mode==='max'?rate<=targetPct:rate>=targetPct;const status=pass?'On target':'Below target';const color=pass?'#1f7a3f':'#9a3412';return `<div class='pill'><div class='tiny'>${label}</div><div style='font-size:28px;font-weight:800'>${rate}%</div><div class='mini muted'>${num}/${den||0} · target ${mode==='max'?'≤':'≥'}${targetPct}%</div><div class='mini' style='color:${color};font-weight:700'>${status}</div></div>`;}
...
['M0/M1',`${sb.m0_cases||0}/${sb.m1_cases||0}`,'M stage distribution']
 ...
 el('qualityBenchmarks').innerHTML=[
  benchCard('Nodes ≥12',ge12,total,85,'min'),
  benchCard('Venous invasion detection',emvi,total,25,'min'),
  benchCard('CRM involved rate',r1,total,20,'max'),
  benchCard('pT3/pT4 case-mix',pt3,total,40,'min')
 ].join('');
